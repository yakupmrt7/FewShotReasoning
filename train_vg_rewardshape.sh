...
#SBATCH --job-name=grpo_vg_rewardshape_qwen35
#SBATCH --output=grpo_vg_rewardshape-%j.out
#SBATCH --error=grpo_vg_rewardshape-%j.err
...
export WANDB_RUN_NAME=Qwen3.5-VL-VG-RewardShape-GRPO-$(date +%Y-%m-%d-%H-%M-%S)
...
echo "Phase C ablation: reward reshaping only, same base/data/hyperparams as the original VG"
echo "GRPO run. Uses the new 'accuracy_shaped' reward (grpo.py:accuracy_reward_vg_shaped):"
echo "  - format is a hard gate (0 reward if <reasoning>/<answer> tags don't match), not a"
echo "    separate +0.2 bonus that sat saturated the whole original run"
echo "  - bbox score is dense IoU + a threshold bonus at IoU>=0.5, matching the eval metric"
echo "    (precision@IoU 0.5) instead of paying equally for IoU gains that don't cross it"
echo "Base model: qwen35-cot-vg-merged (same as the original VG GRPO run)"
echo "Dataset: VHM_dataset_grpo_vg_only_2k (same 843 examples as the original VG GRPO run)"
echo "Same stabilized hyperparams otherwise (beta=0.01, lr=1e-6, warmup=0.1)"
echo "=============================="
