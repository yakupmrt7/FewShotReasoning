...
#SBATCH --job-name=grpo_cotall_vqa
#SBATCH --output=grpo_cotall_vqa-%j.out
#SBATCH --error=grpo_cotall_vqa-%j.err
...
export WANDB_RUN_NAME=Qwen3.5-VL-VQA-CoTall-GRPO-$(date +%Y-%m-%d-%H-%M-%S)
...
echo "Base model: qwen35-cotall-vqa-merged (Qwen3.5-VL, SFT + pooled-CoT on CLS+VQA+VG)"
echo "Dataset: VHM_dataset_grpo_vqa_only_2k_363"
echo "=============================="
...
python -u -m torch.distributed.run \
    ...
    --model_name_or_path /arf/scratch/aalatan/Re-CoT/Qwen-VL-Series-Finetune/output/qwen35-cotall-vqa-merged \
