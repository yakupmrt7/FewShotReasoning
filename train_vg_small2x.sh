...
#SBATCH --job-name=grpo_vg_small2x_qwen35
#SBATCH --output=grpo_vg_small2x-%j.out
#SBATCH --error=grpo_vg_small2x-%j.err
...
export WANDB_RUN_NAME=Qwen3.5-VL-VG-Small2x-GRPO-$(date +%Y-%m-%d-%H-%M-%S)
...
echo "Dataset: VHM_dataset_grpo_vg_only_2k_small_2x (843 original + 160 small-box duplicates = 1003; small-object share ~19% -> ~32%)"
echo "Goal: recover VG precision lost in the CoT stage, with extra gradient signal on small objects (worst-performing size bucket: -4.53pp vs SFT-only in the first GRPO run)"
...
python -u -m torch.distributed.run \
    ...
    --dataset_name /arf/scratch/aalatan/VHM_dataset_grpo_vg_only_2k_small_2x \
