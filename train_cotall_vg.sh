...
#SBATCH --job-name=grpo_cotall_vg
#SBATCH --output=grpo_cotall_vg-%j.out
#SBATCH --error=grpo_cotall_vg-%j.err
...
export WANDB_RUN_NAME=Qwen3.5-VL-VG-CoTall-GRPO-$(date +%Y-%m-%d-%H-%M-%S)
...
echo "Base model: qwen35-cotall-vg-merged (Qwen3.5-VL, SFT + pooled-CoT on CLS+VQA+VG)"
echo "Dataset: VHM_dataset_grpo_vg_only_2k"
...
python -u -m torch.distributed.run \
    ...
    --model_name_or_path /arf/scratch/aalatan/Re-CoT/Qwen-VL-Series-Finetune/output/qwen35-cotall-vg-merged \
